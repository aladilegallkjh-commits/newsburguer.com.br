...
export interface PixPayload {
  ...
  txid?: string;        // Identificador da transação (opcional, só alfanumérico)
  descricao?: string;   // (não usado - alguns bancos rejeitam)
}
...
export function gerarPixCopiaECola(params: PixPayload): string {
  ...
  // txid: APENAS alfanumérico, máx 25 chars
  // Se não fornecido ou só tiver caracteres inválidos (ex: '***'), NÃO inclui o campo 62
  // Isso é mais compatível com bancos do que usar '***' no campo 62
  const txid = (params.txid || '').replace(/[^a-zA-Z0-9]/g, '').substring(0, 25);

  // Merchant Account Information (MAI) - Dados do destinatário PIX
  // Campo 26: GUI 'br.gov.bcb.pix' + chave
  const gui = field('00', 'br.gov.bcb.pix');
  const chaveField = field('01', params.chave);
  const mai = field('26', gui + chaveField);

  // Campo 62 (Additional Data Field) só é incluído se houver txid alfanumérico válido
  // Sem campo 62, o QR é estático anônimo - compatível com todos os bancos
  const additionalData = txid ? field('62', field('05', txid)) : '';

  // Monta payload sem CRC
  const payload =
    field('00', '01') +           // Payload Format Indicator
    field('01', '11') +           // Point of Initiation: 11 = QR Estático
    mai +                          // Merchant Account Information (PIX)
    ...
    additionalData +              // Additional Data Field (txid, se houver)
